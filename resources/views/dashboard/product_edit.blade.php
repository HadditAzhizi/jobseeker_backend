      ...
      <div class="container-fluid">
          <div class="card mb-3 shadow no-b r-0">
            ...
            <div class="card-body"> 
              ...
              <form id="form-simpan" action="/product/edit" method="post" enctype="multipart/form-data">
              @csrf
              @method('POST')
              <center>
                      <img src="/uploads/{{ $product[0]->gambar }}" height="120">
                    </center>
                  ...
                  <div class="form-group">
                      ...
                      <input type="text" name="nama" id="nama" class="form-control" value="{{ $product[0]->nama }}"> 
                  </div> 
                 <div class="form-group">
                      <label class="control-label mb-10">Deskripsi</label>
                      <textarea name="deskripsi" id="deskripsi">{{ $product[0]->deskripsi }}</textarea>
                  </div> 
                 <div class="form-group">
                      <label class="control-label mb-10">Harga</label>
                      <input type="number" name="harga" id="harga" class="form-control" value="{{ $product[0]->harga }}">  
                  </div> 
                 <div class="form-group">
                      <label class="control-label mb-10">Stock</label>
                      <input type="number" name="stock" id="stock" class="form-control" value="{{ $product[0]->stock }}">  
                  </div> 
                    <input type="text" name="img_lama" value="{{ $product[0]->gambar }}" hidden="">
                    <input type="text" name="id" value="{{ $product[0]->id_product }}" hidden="">
                   <div class="form-group">
                      ...
                      <select class="form-control select2" name="prod_kateg">
                        @foreach ($dt_kateg as $kategori)
                          <?php
                            if($kategori->id==$product[0]->kateg_id)
                            {
                              $is_select = "selected";
                            }
                            else
                          ...
                          ?>
                          <option selected="{{$is_select}}" value="{{ $kategori->id }}">{{ $kategori->nama }}</option>
                        @endforeach
                      </select>
                  </div> 
